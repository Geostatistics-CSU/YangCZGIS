...
#include "./src/service/YangCZFXYPyThread.h"
#include "./src/service/yangczfxydialog.h"
#include "./src/service/coordinateproject.h"
#include "./src/service/reservesestimatesdialog.h"
#include "./src/service/AmountPyThread.h"
#include "./src/service/abnormalgpsdialog.h"
...
private:
    ...
    STYangCZServiceDialog* _mstyangcz;
    CoYangCZServiceDialog* _mcoyangcz;
    AmoebaGServiceDialog* _mamoebag;
    HTMSCANServiceDialog* _mhtmscan;
...
private slots:
    ...
    void onOYangCZParamsSended(QList<double>observed_data, QList<ObPtXYZ> obpts, QList<ObPtXYZ> un_obpts, double c, int k, int dim, QString outputPath, bool e, bool islog, bool isTIF);
    void onNewOYangCZ();
    void onActionYangCZF();
    void onYangCZFParamsSended(QList<double>observed_data, QList<ObPtXYZ> obpts, int k, int dim, QString path);
    void onNewYangCZF();
    void onActionGYangCZ();
    void onGYangCZParamsSended(QList<double>observed_data, QList<ObPtXYZ> obpts, QList<double>train_index_matrix, QList<double>test_index_matrix, QList<ObPtXYZ> un_obpts, double c, int k, int k2, int dim, QString outputPath, bool e, bool islog, bool isTIF);
    void onNewGYangCZ();
    void onActionSTYangCZ();
    void onSTYangCZParamsSended(QList<QList<double>>observed_data, QList<ObPtXYZ> obpts, double c_s, double c_t, int k_s, int k_t, int dim, QString outputPath, bool islog);
    void onNewSTYangCZ();
    void onActionCoYangCZ();
    void onCoYangCZParamsSended(QList<double> observed_data, QList<ObPtXYZ> co_observed_data, QList<ObPtXYZ> obpts, QList<ObPtXYZ> co_obpts, QList<ObPtXYZ> un_obpts, QList<double> c_list, int k, int dim, int n_co, QString outputPath, bool isnugget, bool islog, bool isTIF);
    void onNewCoYangCZ();
    ...
    void onNewHtmscan();
    void onActionRuleYangCZXY();
    void onYangCZFXYParamsSended(QList<QList<double>>observed_data, double r_d, double c_d, double c, int k, int dim);

    void onActionTransformXY();

    void onActionAmount();
    void onAmountParamsSended(QList<double> obPt_val, QList<ObPtXYZ> obPts_c, QList<ObPtXYZ> un_obPts_c, double c, int k, int kr, double l_r, double l_c, int n_r, int n_c, double dz,
        double pa, double pb, double pc, QString outpath, bool error, double X0, double Y0, double A_0);
    void onImport_Amount(PyObject* result_re);
};