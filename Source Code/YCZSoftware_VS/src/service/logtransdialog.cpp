...
void LogTransDialog::onBtnDrawClicked()
{
    ...
    // 确定字段名称的最大长度限制（例如10个字符）
    int maxFieldLength = 10;

    // 生成新字段名称并检查长度
    QString logFieldName = fieldName + "_log";
    if (logFieldName.length() > maxFieldLength) {
        logFieldName = fieldName.left(maxFieldLength - 4) + "_log"; // 截断 fieldName，以确保 "_log" 后缀可以添加
    }
    ...
    layer->commitChanges(); // 保存修改
    //QgsVectorLayerEditUtils::stopEditing(layer); // 停止编辑
