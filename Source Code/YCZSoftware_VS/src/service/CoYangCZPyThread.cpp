...
CoYangCZPyThread::CoYangCZPyThread(QList<double> observed_data, QList<ObPtXYZ> co_observed_data, QList<ObPtXYZ> obpts, QList<ObPtXYZ> co_obpts, QList<ObPtXYZ> un_obpts, QList<double> c_list, int k, int dim, int n_co, QString outputPath, bool isnugget, bool islog, bool isTIF)
{
...
void CoYangCZPyThread::setParams()
{
	...
	PyTuple_SetItem(_mParams, 11, Py_BuildValue("b", _mis_log));
	PyTuple_SetItem(_mParams, 12, Py_BuildValue("b", _misTif));
