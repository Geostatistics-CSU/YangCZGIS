#include "amoebagservicedialog.h"
#include "ui_amoebagservicedialog.h"

AmoebaGServiceDialog::AmoebaGServiceDialog(QgsProject* project, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AmoebaGServiceDialog)
{
    ui->setupUi(this);
    this->setStyleSheet("QWidget { font-size: 17px; }");
    ui->cmb_choose->setCurrentIndex(0);
    ui->lineNeiPath->setEnabled(true);
    ui->btnBrowseNei->setEnabled(true);
    ui->cmb_s->setEnabled(true);
    ui->dsb_l->setEnabled(false);
    //ui->label_area->setEnabled(false);
    //ui->cmbRangePath->setEnabled(false);

    this->initUI(project->layers<QgsVectorLayer*>());
    connect(ui->cmb_choose, &QComboBox::currentTextChanged, this, &AmoebaGServiceDialog::InputChanged);
    connect(ui->btnComfirm, &QPushButton::clicked, this, &AmoebaGServiceDialog::onBtnConfirmClicked);
    connect(ui->btnCancel, &QPushButton::clicked, this, &AmoebaGServiceDialog::onClose);
}

AmoebaGServiceDialog::~AmoebaGServiceDialog()
{
    delete ui;
}

void AmoebaGServiceDialog::initUI(QVector<QgsVectorLayer*> pjLyr)
{
    dim = 0;
    ...
    connect(ui->cmbObPath, &QComboBox::currentTextChanged, this, &AmoebaGServiceDialog::onCmbObPathChange);
    connect(ui->btnBrowseNei, &QPushButton::clicked, this, &AmoebaGServiceDialog::onBtnBrowseNeiClicked);
    connect(ui->btnBrowseOutput, &QPushButton::clicked, this, &AmoebaGServiceDialog::onBtnBrowseOutputClicked);
}

void AmoebaGServiceDialog::InputChanged()
{
    ...
    {
        ui->lineNeiPath->setEnabled(false);
        ui->btnBrowseNei->setEnabled(false);
        ui->cmb_s->setEnabled(false);
        ui->dsb_l->setEnabled(true);
        //ui->label_area->setEnabled(true);
        //ui->cmbRangePath->setEnabled(true);
    }
    else
    {
        ui->lineNeiPath->setEnabled(false);
...
void AmoebaGServiceDialog::onCmbObPathChange()
{
    ui->cmbObVal->clear();
    ui->cmb_x->clear();
    ui->cmb_y->clear();
    ...
    connect(ui->cmb_z, &QComboBox::currentTextChanged, this, &AmoebaGServiceDialog::onXYZChanged);
    connect(ui->cmb_y, &QComboBox::currentTextChanged, this, &AmoebaGServiceDialog::onXYZChanged);
}

void AmoebaGServiceDialog::onBtnBrowseOutputClicked()
{
...
void AmoebaGServiceDialog::onBtnBrowseNeiClicked()
{
    // 设置文件对话框的标题
...
void AmoebaGServiceDialog::onXYZChanged()
{
...
void AmoebaGServiceDialog::onBtnConfirmClicked()
{
...
void AmoebaGServiceDialog::onClose()
{
    this->close();
}
