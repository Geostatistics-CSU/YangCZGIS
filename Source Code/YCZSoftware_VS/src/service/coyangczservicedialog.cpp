    ...
    ui(new Ui::CoYangCZServiceDialog)
{
    ...
    connect(ui->btnCancel, &QPushButton::clicked, this, &CoYangCZServiceDialog::onClose);
...
void CoYangCZServiceDialog::onBtnConfirmClicked()
{
    ...
    else
    {
        ...
        if (ui->cmb_y->currentIndex() == 0) {
            ...
            if (n_x < 2 || n_x>20000 || n_y < 2 || n_y>20000) {
                QMessageBox::critical(nullptr, "Error about input grid size", "Input grid size is too small or too big!");
                return;
            }
    ...
    if (run_code) {
        ...
        emit sendPyParams(obPt_val, co_obPt_val, obPts_c, co_obPts_c, un_obPts_c, c_list, k_num, dim, num_co, outputPath, is_nug, is_log, isTIF);
        emit getOutPath(outputPath, isTIF);
    }
}

void CoYangCZServiceDialog::onClose()
{
    this->close();
}
