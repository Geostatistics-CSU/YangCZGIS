...
YangCZPyThread::YangCZPyThread(QList<double> observed_data, QList<ObPtXYZ> obpts, QList<ObPtXYZ> un_obpts, double c, int k, int dim, QString outputPath, bool e, bool islog, bool isTIF)
{
...
void YangCZPyThread::setParams()
{
	...
	PyTuple_SetItem(_mParams, 8, Py_BuildValue("b", _mislog));
	PyTuple_SetItem(_mParams, 9, Py_BuildValue("b", _misTIF));
