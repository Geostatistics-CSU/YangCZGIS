    ...
    ui(new Ui::GYangCZServiceDialog)
{
    ...
    connect(ui->btnCancel, &QPushButton::clicked, this, &GYangCZServiceDialog::onClose);
}
...
void GYangCZServiceDialog::initUI(QVector<QgsVectorLayer*> pjLyr)
{
    ...
    QWidget* subWidget = findChild<QWidget*>("widget");
    if (subWidget) {
        subWidget->setStyleSheet(R"(
        #widget {
            border: 1px solid gray;          /* »ÒÉ«±ß¿ò */
            background-color: rgba(255, 255, 255, 0.9); /* ±³¾°Îª»Ò°×É«£¬40%Í¸Ã÷¶È */
        }
    )");
    }
    QWidget* subWidget_2 = findChild<QWidget*>("widget_2");
    if (subWidget_2) {
        subWidget_2->setStyleSheet(R"(
        #widget_2 {
            border: 1px solid gray;          /* »ÒÉ«±ß¿ò */
            background-color: rgba(255, 255, 255, 0.9); /* ±³¾°Îª»Ò°×É«£¬40%Í¸Ã÷¶È */
        }
...
void GYangCZServiceDialog::InputChanged()
{
    ...
    if (choose == 0)
    {
        ...
        ui->cmb_unknow->setEnabled(true);
        //ui->label_unknow_c->setVisible(true);
        //ui->xyz_widget->setVisible(true);
        //ui->label_grid->setVisible(false);
        ui->line_size->setEnabled(false);
        //ui->label_label_un->setVisible(true);
        ui->cmbObUnLab->setEnabled(true);
        //ui->label_area->setVisible(false);
        //ui->cmbRangePath->setVisible(false);
    }
    else if (choose == 1)
    {
        ui->label_grid->setEnabled(true);
        ui->line_size->setEnabled(true);
        //ui->label_unknow->setVisible(false);
        ui->cmb_unknow->setEnabled(false);
        //ui->label_unknow_c->setVisible(false);
        //ui->xyz_widget->setVisible(false);
        //ui->label_label_un->setVisible(true);
        ui->cmbObUnLab->setEnabled(true);
    ...
    else
    {
        ...
        ui->cmb_unknow->setEnabled(false);
        //ui->label_unknow_c->setVisible(false);
        ui->label_grid->setEnabled(false);
        ui->line_size->setEnabled(false);
        //ui->xyz_widget->setVisible(false);
        //ui->label_label_un->setVisible(false);
        ui->cmbObUnLab->setEnabled(false);
        //ui->label_area->setVisible(false);
        //ui->cmbRangePath->setVisible(false);
    }
}
...
void GYangCZServiceDialog::onBtnConfirmClicked()
{
        ...
        if (ui->cmb_y->currentIndex() == 0) {
            ...
            while (obIter_u.nextFeature(obFeat_u)) {
                ...
                if (ui->cmbObUnLab->currentIndex() > 0)
                    test_lab = obFeat_u.attribute(ui->cmbObUnLab->currentIndex() - 1).toDouble(&valOk);
                if (!valOk) {
                    QString XStr_u = obFeat_u.attribute(ui->cmb_x_u->currentIndex()).toString();
                    X_u = XStr_u.toDouble(&valOk);
                    if (ui->cmbObUnLab->currentIndex() > 0)
                    {
                        QString test_lab_str = obFeat_u.attribute(ui->cmbObUnLab->currentIndex() - 1).toString();
            ...
            while (obIter.nextFeature(obFeat)) {
                ...
                double train_lab = 1;
                if (ui->cmbObLab->currentIndex() > 0)
                    train_lab = obFeat.attribute(ui->cmbObLab->currentIndex() - 1).toDouble(&valOk);
    ...
    if (run_code) {
        this->close();
        emit begin("GYangCZ");
        emit sendPyParams(obPt_val, obPts_c, obPt_train, obPt_test, un_obPts_c, c_val, k_num, k2_num, dim, outputPath, error_1, islog, isTIF);
        emit getOutPath(outputPath, isTIF);
    }
  
}

void GYangCZServiceDialog::onClose()
{
    this->close();
}
