    ...
    ui(new Ui::OYangCZServiceDialog)
{
    ...
    connect(ui->btnCancel, &QPushButton::clicked, this, &OYangCZServiceDialog::onClose);
}
...
void OYangCZServiceDialog::onBtnConfirmClicked()
{
    ...
    if (run_code) {
        ...
        emit sendPyParams(obPt_val, obPts_c, un_obPts_c, c_val, k_num, dim, outputPath, error_1, islog, isTIF);
        emit getOutPath(outputPath, isTIF);
    }
}

void OYangCZServiceDialog::onClose()
{
    this->close();
}
