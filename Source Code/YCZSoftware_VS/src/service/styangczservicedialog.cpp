...
void STYangCZServiceDialog::onBtnConfirmClicked()
{
    ...
    if (ui->cmb_y->currentIndex() == 0) {
        ...
        while (obIter.nextFeature(obFeat)) {
            ...
            for (int i = ui->cmbObVal_b->currentIndex(); i <= ui->cmbObVal_e->currentIndex(); i++)
            {
                QString attributeValue = obFeat.attribute(i).toString();
                if (attributeValue.isEmpty()) {
                    val_i.append(std::numeric_limits<double>::quiet_NaN());
                }
                else {
                    double val = attributeValue.toDouble(&valOk);
                    if (valOk)
                    {
                        val_i.append(val);
                    }
            ...
            if (!valOk) {
                QString XStr = obFeat.attribute(ui->cmb_x->currentIndex()).toString();
                QString YStr = obFeat.attribute(ui->cmb_y->currentIndex()).toString();
                X = XStr.toDouble(&valOk);
    ...
    if (run_code) {
        ...
        emit sendPyParams(obPt_val, obPts_c, cs_val, ct_val, ks_num, kt_num, dim, outputPath, islog);
